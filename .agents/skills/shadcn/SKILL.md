...
### Desktop Design (OmniDesk) → [desktop-design.md](./rules/desktop-design.md)
- **Compact UI only**: Use `size="sm"`, `h-8`, `p-2`/`p-3`, `gap-1`/`gap-2`. Never use mobile-first spacing.
- **Local scrolling**: Always wrap list/content areas in `<ScrollArea>`, no window scrolling.
