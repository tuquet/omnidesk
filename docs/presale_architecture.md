...
4. **Thu thập Logs (Automa -> Engine):** Ngay khi trình duyệt mở, Automa được kích hoạt và bắt đầu chạy Workflow. Xuyên suốt quá trình chạy, Automa liên tục bắn Logs (trạng thái, lỗi) về cổng thu thập của Omni Engine qua WebSocket.
5. **Proxy tới Nguồn Chân lý (Engine -> Studio):** Omni Engine đóng vai trò "Proxy", gọi các REST API sang Omni Studio (`Cổng 1422`) để tạo Runs và ghi Logs. Tuân thủ tuyệt đối nguyên tắc Microservices: Engine không chạm trực tiếp vào DB của Studio.

### 2.3. Luồng Kiểm định & Hợp nhất Kịch bản (QC & Contributor Workflow)

Lấy cảm hứng từ quy trình **Git Flow / Pull Request** trên GitHub, OmniDesk thiết lập một ranh giới an toàn tuyệt đối giữa môi trường thử nghiệm (Local Workspace) và môi trường sản xuất (Production / Omni Studio):

1. **Kéo Kịch bản (Pull):** Khi đội ngũ QA/QC hoặc Developer mở **Omni Engine (Orchestrator)**, họ kéo (Pull) kịch bản gốc từ Studio về Database nội bộ (IndexedDB) của Extension Automa trên trình duyệt.
2. **Vá lỗi & Chạy thử (Local Debugging):** QC có thể tùy ý thay đổi, kéo thả, thêm node và chạy test trực tiếp trên máy của họ mà không làm ảnh hưởng đến luồng Production đang chạy kiếm tiền.
3. **Đề xuất Hợp nhất (Submit for Review / Pull Request):** Sau khi kịch bản chạy mượt mà (100% Pass), QC bấm "Submit". Kịch bản được đóng gói và đẩy qua API lên Omni Studio ở trạng thái `Draft` hoặc `Pending_Review`.
4. **Phê duyệt & Hợp nhất (Approve & Merge):** Quản lý (Admin/Tech Lead) vào giao diện "Review Queue" trên Omni Studio, kiểm tra sự thay đổi (Diff), duyệt (Approve) và hợp nhất (Merge) thành một phiên bản (Version) mới chính thức.
...
- [ ] **Review Queue & Merge**: Quản lý phiên bản kịch bản, cho phép phê duyệt và hợp nhất (Merge) các bản cập nhật do QC/Contributor đẩy lên từ Orchestrator.
...
- [x] **Orchestration UI (Command Center)**: Giao diện cho phép chọn 1 Workflow, gắn vào 1 hoặc nhiều Profiles, thiết lập lịch chạy (Run Once/Cronjob).
- [x] **Omni Engine (Orchestrator)**: Đóng vai trò nhạc trưởng, nhận lệnh "Run", tự động cài Automa và gọi API của Profile để mở trình duyệt.
- [x] **Log Collection & Microservices Proxy**: Automa tự động thực thi kịch bản, bắn Real-time Logs về Engine qua WebSocket, Engine proxy gọi API ghi dữ liệu sang Studio.
- [ ] **Contributor Workspace**: Giao diện cho phép QC dễ dàng submit (Pull Request) kịch bản đã vá lỗi lên Omni Studio.
- [ ] **Analytics Dashboard**: Omni Engine phân tích các phiên chạy (Sessions) và hiển thị biểu đồ báo cáo trực quan.
