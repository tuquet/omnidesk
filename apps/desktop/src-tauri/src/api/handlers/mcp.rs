            ...
            if let Some(params) = req.params {
                if let Some(uri) = params.get("uri").and_then(|u| u.as_str()) {
                    if uri == "sqlite://user_installed_apps" {
                        // Return fake/real data depending on DB
                        let apps: Vec<String> = vec!["OmniVault".to_string(), "OmniChat".to_string()];
                        response.result = Some(serde_json::json!({
                            "contents": [
                                {
                                    "uri": uri,
                                    "mimeType": "application/json",
                                    "text": serde_json::to_string(&apps).unwrap()
                                }
                            ]
                        }));
                    } else {
            ...
            if let Some(params) = req.params {
                ...
                match name {
                    "get_keyring" => {
                        ...
                        // Fake retrieving from keyring for demonstration context
                        response.result = Some(serde_json::json!({
                            "content": [
                                {
                                    "type": "text",
                                    "text": format!("Retrieved secret for {}/{} securely from OS Keyring.", service, account)
                                }
                            ]
                        }));
                    }
                    "sync_command" => {
                        ...
                        response.result = Some(serde_json::json!({
                            "content": [
                                {
                                    "type": "text",
                                    "text": format!("Queued command '{}' to sync_queue.", cmd)
                                }
                            ]
                        }));
                    }
