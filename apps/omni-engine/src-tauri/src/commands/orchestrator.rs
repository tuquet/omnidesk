...
pub async fn ensure_automa_extension(app: AppHandle) -> Result<String, String> {
    let _ = app.emit(omni_tauri_core::constants::E2E_LOG_EVENT, "[SYSTEM] Checking Automa Extension...");
    ...
    if automa_dir.exists() && automa_dir.join("manifest.json").exists() {
        let _ = app.emit(omni_tauri_core::constants::E2E_LOG_EVENT, "[SYSTEM] Automa Extension is already unpacked and ready.");
        return Ok(automa_dir.to_string_lossy().to_string());
    }

    let _ = app.emit(omni_tauri_core::constants::E2E_LOG_EVENT, "[SYSTEM] Downloading Automa Extension...");
    ...
    let _ = app.emit(omni_tauri_core::constants::E2E_LOG_EVENT, "[SYSTEM] Download complete. Unzipping...");
    ...
    let _ = app.emit(omni_tauri_core::constants::E2E_LOG_EVENT, "[SYSTEM] Automa Extension unzipped successfully.");
