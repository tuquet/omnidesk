...
const loadState = (): LauncherState => {
  try {
    ...
    console.error('Failed to load launcher state', e instanceof Error ? e.message : String(e));
  }
...
launcherStore.subscribe(() => {
  try {
    ...
    console.error('Failed to save launcher state', e instanceof Error ? e.message : String(e));
  }
});
