...
export default function AutomaDashboard() {
  ...
  const fetchWorkflows = async () => {
    try {
      ...
      const { data, error } = await supabase.from('e2e_workflows').select('*');
      if (data) {
        setWorkflows(data);
      }
    } catch (e) {
  ...
  const runTests = async () => {
    if (!isTauri) {
      setLogs(prev => [...prev, '[Web] Execution only supported in Desktop app']);
      return;
    }
    
    setIsRunning(true);
    setLogs(['[System] Starting E2E Orchestrator...']);
    
    try {
      await invoke('run_e2e_orchestrator');
      // Note: The orchestrator runs async, so it doesn't block.
      // We rely on logs to see progress.
    } catch (e: any) {
