...
pub async fn run_project_script(
    ...
    let mut project_path = std::env::var("NHAATELIER_CONTENT_PATH")
        .ok()
        .map(PathBuf::from);

    // If env var is not set, dynamically locate the nhaateliertattoo workspace
    if project_path.is_none() {
        ...
        for _ in 0..10 {
            ...
            let sibling_path = current.join("../nhaateliertattoo/apps/nhaatelier-content");
            if sibling_path.exists() {
                project_path = Some(sibling_path);
                break;
            }
    ...
    let project_path = project_path.ok_or_else(|| {
        AppError::NotFound(
            "WordPress sync workspace directory was not found. Please configure the NHAATELIER_CONTENT_PATH environment variable."
                .to_string(),
        )
    })?;
